...
float ca,sa,cb,sb; //cameraangles;
//Forwards
...
DWORD WINAPI ProcessAudioThread2 (LPVOID ptr) // Double sounds proceed thread
{
   for (;;) {
if (DELAY)
...
int InitDirectSound( HWND hwnd)
{
   ...
   PrintLog("ok\n");
   if (DELAY) {  
    PrintLog("Create audio thread #2..");
    hAudioThread2 = CreateThread(NULL, 0, ProcessAudioThread2, NULL, 0, &AudioTId2);
    SetThreadPriority(hAudioThread2, THREAD_PRIORITY_HIGHEST);
    PrintLog("ok\n");
   }
...
{   
   ...
   hwndApp = hw;	
   hlog = hlogg;
   wsprintf(logtt,"\nDirectSound3D(native) audio driver ");
   PrintLog(logtt);
   if (DELAY) wsprintf(logtt,"(with Reverb enable) Beta build v%d.%d\n", version>>16, version & 0xFFFF);   else 
   wsprintf(logtt,"(Beta 4) v%d.%d\n", version>>16, version & 0xFFFF);
   PrintLog(logtt);   
...
void Audio_Shutdown(void)
{
 ...
 if (DELAY) {
  PrintLog(" Terminate thread 2\n");
  TerminateThread(hAudioThread2,0);
 }
...
//////////////////
bool DuplicateInit(int num)
{ HRESULT err;
  //PrintLog("Double buffering init\n");
  sounds[num].drun = false;

  err = lpDS->DuplicateSoundBuffer(sounds[num].lpDSBuf,&dsounds[num].lpDSBuf);
  if( err != DS_OK ) { ParseError("DuplicateSoundBuffer error",err); return false;}

  if (sounds[num].lpDS3DBuf) {
   err = dsounds[num].lpDSBuf->QueryInterface(IID_IDirectSound3DBuffer, (void**)&dsounds[num].lpDS3DBuf);
   if( err != DS_OK ) { wsprintf(logtt,"DoubleSounds QueryInterface 3D\n"); ParseError(logtt,err); return false;}
  }
  return true;
}
...
//////////
/*
void MulVectorsVect(D3DVECTOR& v1, D3DVECTOR& v2, D3DVECTOR& r )
{
...
void AnglesToVector(float alfa,float beta, D3DVECTOR& fv, D3DVECTOR& tv)
{
  //sinA = sin( aCamera ); cosA = cos( aCamera );
  //sinB = sin( bCamera ); cosB = cos( bCamera );

  D3DVECTOR tempvec;
  D3DVECTOR v010;

  fv.x = (float)(cos(beta)*sin(alfa));     
  fv.y = (float)(sin(beta));
  fv.z = (float)(-cos(alfa)*cos(beta));    
  ...
  MulVectorsVect(fv, v010, tempvec);
  MulVectorsVect(tempvec, fv, tv);
...
{
  //alfa=3.1415926-alfa;
  fv.x = (float)(-sin(alfa)*cos(beta));  
  fv.y = (float)(-sin(beta));
  fv.z = (float)(cos(alfa)*cos(beta)); 
  
  tv.x = (float)(-(sin(beta)*sin(alfa)));
  tv.y = (float)(cos(beta));
  tv.z = (float)((sin(beta)*cos(alfa)));
}*/

/*void UpdateListener3D(D3DVALUE x_,D3DVALUE y_,D3DVALUE z_, D3DVECTOR front,D3DVECTOR top)
{
	if (lpDSListener3D) {
	 //lpDSListener3D->SetOrientation( front.x,front.y,front.z, top.x,top.y,top.z, DS3D_IMMEDIATE); //DS3D_IMMEDIATE
    lpDSListener3D->SetOrientation( 0,0,1,0,1,0, DS3D_IMMEDIATE); //DS3D_IMMEDIATE
    //lpDSListener3D->CommitDeferredSettings();
	}
}*/
...
int volu;
	...
	if (DELAY)
	if (dsounds[num].lpDS3DBuf) {
	 if (!play) dsounds[num].lpDS3DBuf->SetMinDistance(MinDistance,DS3D_IMMEDIATE);
	 if (!play) dsounds[num].lpDS3DBuf->SetMaxDistance(MaxDistance,DS3D_IMMEDIATE);
	 dsounds[num].lpDS3DBuf->SetPosition(xx,yy,zz,DS3D_IMMEDIATE);
	}
	}
...
if (play) {
 ...
 if (DELAY) if (dsounds[num].lpDSBuf) dsounds[num].lpDSBuf->SetVolume(volu);
 
 sounds[num].lpDSBuf->Play(0,0,0);
 if (DELAY) sounds[num].drun = true;
}
...
void Audio_MixChannels()
{
    ...
    for( int i = 0; i < MAX_CHANNEL; i++ )        
 	...
 	if (DELAY) {
			if (dsounds[i].lpDSBuf) dsounds[i].lpDSBuf->Stop();
            if (dsounds[i].lpDS3DBuf) dsounds[i].lpDS3DBuf->Release();
			dsounds[i].lpDS3DBuf = NULL;
			if (dsounds[i].lpDSBuf) dsounds[i].lpDSBuf->Release();
            dsounds[i].lpDSBuf = NULL;}
