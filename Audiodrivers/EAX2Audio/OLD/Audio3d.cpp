...
int InitDirectSound( HWND hwnd)
{
   ...
   if (!EAXpresent()) 
   {
	 PrintLog("EAX 1.0/2.0 is not supported. Running in muted mode. Please choose another sound output device\n"); 
   }
...
{   
   hwndApp = hw;
   hlog = hlogg;
   wsprintf(logtt,"\nCreative EAX 1.0/2.0 audio driver (beta 3) %d.%d\n", version>>16, version & 0xFFFF);
   PrintLog(logtt);
   if( !InitDirectSound( hwndApp ) ) PrintLog("EAX 2.0 init failed\n");
...
ULONG dwFlags = 0;// EAXBUFFERFLAGS_ROOMAUTO|EAXBUFFERFLAGS_DIRECTHFAUTO | EAXBUFFERFLAGS_ROOMHFAUTO;//( EAXBUFFERFLAGS_DIRECTHFAUTO | EAXBUFFERFLAGS_ROOMAUTO |  EAXBUFFERFLAGS_ROOMHFAUTO);
EAXBUFFERPROPERTIES EAXbuffer = {0, 0, 0, 0, 0.0f,  0,0,0,0,0, 0, 1.0f, dwFlags};
...
if (EAX1){
 ...
 if (EAX2) 
	 if FAILED(sounds[num].pEmitterPropertySet->Set(DSPROPSETID_EAX_BufferProperties,DSPROPERTY_EAXBUFFER_ALLPARAMETERS ,NULL,0, &EAXbuffer,sizeof(EAXBUFFERPROPERTIES)))   PrintLog("Set() error\n");
...
EAX_REVERBPROPERTIES generic1      = {EAX_ENVIRONMENT_GENERIC,0.0f,0.0f,0.0f};
...
ULONG dwLFlags =                  (EAXLISTENERFLAGS_DECAYTIMESCALE |        
...
if (EAX2) {
	ULONG envID = EAX_ENVIRONMENT_FOREST;
	ULONG Volume = -1000;
	float Decay  =  2.124f;
	float Damping = 0.44f;
	
    if FAILED(lpPropertySet->Set(DSPROPSETID_EAX_ListenerProperties,DSPROPERTY_EAXLISTENER_ENVIRONMENT,NULL,0, &envID, sizeof(long))) PrintLog("EnvSet fail");
    if FAILED(lpPropertySet->Set(DSPROPSETID_EAX_ListenerProperties,DSPROPERTY_EAXLISTENER_FLAGS,NULL,0, &dwLFlags, sizeof(long))) PrintLog("FlagSet fail");
    
	if FAILED(lpPropertySet->Set(DSPROPSETID_EAX_ListenerProperties,DSPROPERTY_EAXLISTENER_ROOM,NULL,0, &Volume, sizeof(long))) PrintLog("EnvSet fail");
    if FAILED(lpPropertySet->Set(DSPROPSETID_EAX_ListenerProperties,DSPROPERTY_EAXLISTENER_DECAYTIME,NULL,0, &Decay, sizeof(float))) PrintLog("EnvSet fail");
    if FAILED(lpPropertySet->Set(DSPROPSETID_EAX_ListenerProperties,DSPROPERTY_EAXLISTENER_ENVIRONMENTDIFFUSION,NULL,0, &Damping, sizeof(float))) PrintLog("EnvSet fail");
