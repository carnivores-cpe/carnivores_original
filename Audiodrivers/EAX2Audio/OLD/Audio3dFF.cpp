...
//EAXBUFFERPROPERTIES EAXbuffer = {-50, -50, -200, -200, 1.0f,  0,0,0,0,0, -100, 1.0f, dwFlags};
//EAXBUFFERFLAGS_DIRECTHFAUTO
ULONG dwFlags =  EAXBUFFERFLAGS_ROOMAUTO | EAXBUFFERFLAGS_DIRECTHFAUTO | EAXBUFFERFLAGS_ROOMHFAUTO;//( EAXBUFFERFLAGS_DIRECTHFAUTO | EAXBUFFERFLAGS_ROOMAUTO |  EAXBUFFERFLAGS_ROOMHFAUTO);
...
if (EAX2) {
	...
	EAX2ENV ee;
	ee.envID = EAX_ENVIRONMENT_FOREST;
	ee.Room = - 400;
	ee.Decay  =  2.3f;
	ee.DecayHF = 0.14f;
	ee.Diffusion = 0.3f;
	...
	if FAILED(lpPropertySet->Set(LP,DSPROPERTY_EAXLISTENER_ROOM,NULL,0, &ee.Room, sizeof(long))) PrintLog("VolSet fail");
    
	if FAILED(lpPropertySet->Set(LP,DSPROPERTY_EAXLISTENER_DECAYTIME,NULL,0, &ee.Decay, sizeof(float))) PrintLog("DecaySet fail");
	if FAILED(lpPropertySet->Set(LP,DSPROPERTY_EAXLISTENER_DECAYHFRATIO,NULL,0, &ee.DecayHF, sizeof(ULONG))) PrintLog("RefSet fail");

	if FAILED(lpPropertySet->Set(LP,DSPROPERTY_EAXLISTENER_ENVIRONMENTDIFFUSION,NULL,0, &ee.Diffusion, sizeof(float))) PrintLog("DampSet fail");
